...
class RobotCAMAccelerationStatisticsMetric :
  ...
  override fun plotData() {
    segmentToRobotIdToRobotStateMap.forEach { segmentToRobotIdToRobotStateMap ->
      ...
      val folderName = "robot-cam-acceleration-statistics"
      ...
      robotIdToRobotStates.forEach { (robotId, robotStates) ->
        ...
        val yValues = robotStates.map { it.accelerationCAM ?: 0.0 }
        ...
        plotDataAsLineChart(
            plot =
                getPlot(
                    ...
                    yValues = yValues,
                    "tick",
                    "acceleration (CAM) (m/s^2)",
                    "Acceleration (CAM) for"),
            folder = folderName,
      ...
      plotDataAsLineChart(
          plot = getPlot(combinedValuesMap, "time", "acceleration (CAM)", "Acceleration (CAM) for"),
          folder = folderName,
          subFolder = subFolderName,
