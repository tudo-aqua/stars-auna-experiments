...
class RobotCAMAccelerationAverageStatisticsMetric(
    override val logger: Logger = Loggable.getLogger("robot-cam-acceleration-average-statistics")
) :
  ...
  override fun evaluate(
    ...
    val averageRobotAcceleration =
        robotIdToRobotStateMap.map {
          it.key to it.value.mapNotNull { it.accelerationCAM }.average()
        }
    averageRobotAcceleration.forEach {
      averageAcceleration[it.first] = averageAcceleration.getOrDefault(it.first, 0.0) + it.second
      logFiner(
          "The average acceleration (CAM) of robot with id '${it.first}' in Segment `${segment.getSegmentIdentifier()}` is ${it.second}.")
    }
    tickCount++
  }
  ...
  override fun printState() {
    getState().forEach { (actorId, averageVelocity) ->
      logFine("The average acceleration (CAM) of robot with ID '$actorId' is '$averageVelocity'")
    }
  }
}
