...
val normalLateralOffset =
  predicate(Robot::class) { _, r ->
    (r.lateralOffset ?: 0.0) <= MAX_LATERAL_OFFSET
  }
...
/**
 * Exceeding the maximum distance to the previous vehicle is defined as > [DISTANCE_TO_FRONT_ROBOT_THRESHOLD_LONG].
 */
const val DISTANCE_TO_FRONT_ROBOT_THRESHOLD_LONG: Double = 2.0

/**
 * Exceeding the minimum distance to the previous vehicle is defined as < [DISTANCE_TO_FRONT_ROBOT_THRESHOLD_SHORT].
 */
...
/**
 * Weak acceleration is defined in the interval
 * ([ACCELERATION_ACCELERATION_WEAK_THRESHOLD] .. [ACCELERATION_ACCELERATION_STRONG_THRESHOLD]).
 */
const val ACCELERATION_ACCELERATION_WEAK_THRESHOLD: Double = 0.1

/**
 * Weak deceleration is defined in the interval
 * ([ACCELERATION_DECELERATION_STRONG_THRESHOLD] .. [ACCELERATION_DECELERATION_WEAK_THRESHOLD]).
 */
...
val strongAcceleration =
  predicate(Robot::class) { _, r ->
    rangeStrongAcceleration(r.acceleration ?: 0.0) && rangeStrongAcceleration(r.accelerationCAM ?: 0.0)
  }
...
val weakAcceleration =
  predicate(Robot::class) { _, r ->
    rangeWeakAcceleration(r.acceleration ?: 0.0) && rangeWeakAcceleration(r.accelerationCAM ?: 0.0)
  }


/**
 * No acceleration is defined in the interval
...
val weakDeceleration =
  predicate(Robot::class) { _, r ->
    rangeWeakDeceleration(r.acceleration ?: 0.0) && rangeWeakDeceleration(r.accelerationCAM ?: 0.0)
  }
...
val strongDeceleration =
  predicate(Robot::class) { _, r ->
    rangeStrongDeceleration(r.acceleration ?: 0.0) && rangeStrongDeceleration(r.accelerationCAM ?: 0.0)
  }
//endregion