...
package tools.aqua.stars.auna.metrics.heading
...
/** Metric to calculate the heading statistics of a robot in a segment. */
class RobotHeadingStatisticsMetric(private val plotSegments: Boolean = true) :
  ...
  override fun writePlots() {
    val folderName = "robot-heading-statistics"
          ...
          .map {
            launch {
              ...
              robotIdToRobotStates.forEach { (robotId, robotStates) ->
                ...
                val yValues = robotStates.map { it.rotation.yaw }
                ...
                if (plotSegments) {
                  ...
                  plotDataAsLineChart(
                      plot = getPlot(combinedValuesMap, "time", "heading", "Heading for"),
                      folder = folderName,
                      subFolder = subFolderName,
                  ...
                  finished.incrementAndGet().let { i ->
                    print(
                        "\rWriting Plots for Robot heading: " +
    ...
    allValuesMap.forEach { (legendEntry, values) ->
      plotDataAsLineChart(
          plot =
              getPlot(
                  ...
                  xAxisName = "tick",
                  yAxisName = "heading (m/s^2)",
                  legendHeader = "Heading for"),
          size = 2500 to 500,
          folder = folderName,
          subFolder = "all",
          fileName = "heading_all_${legendEntry}")
      ...
      plotDataAsHistogram(
          plot =
              getPlot(
                  ...
                  yValues = values.second,
                  xAxisName = "heading (m/s^2)",
                  yAxisName = "frequency",
                  legendHeader = "Heading for"),
          folder = folderName,
          subFolder = "all",
          fileName = "heading_hist_all_lin_${legendEntry}")
      ...
      plotDataAsHistogram(
          ...
          folder = folderName,
          subFolder = "all",
          fileName = "heading_hist_all_log_${legendEntry}")
    }

    plotDataAsLineChart(
        plot = getPlot(allValuesMap.toSortedMap(), "tick", "heading (m/s^2)", "Heading for"),
        size = 2500 to 500,
        folder = folderName,
        subFolder = "all",
        fileName = "heading_all_combined")

    println("\rWriting PLots for Robot heading: finished")
  }
      ...
      segmentToRobotIdToRobotStateMap
          .map {
            launch {
              ...
              finished.incrementAndGet().let { i ->
                print(
                    "\rWriting CSV for Robot heading: " +
                        "$i/${segmentToRobotIdToRobotStateMap.size} " +
                        "(${i * 100 / segmentToRobotIdToRobotStateMap.size}%) " +
                        "on ${Thread.currentThread()}")
              }
